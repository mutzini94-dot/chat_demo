...
- **채널 전환** — 투네이션 · 치지직 · 유튜브 (플랫폼별 독립 채팅·공지)
- **클린봇** — 한국어 정규화(NFKC·자모 분해) + 사전 매칭 + 스팸/도배/광고 탐지 → 블라인드(개인 보기 토글)
- **캐시 후원** — 원 단위 캐시 후원, 금액대별 4단계 색상 카드(1천/5천/1만/5만원+)
- **참여형** — 실시간 투표 · 승부예측(응원팀 선택): 진행률 바·집계·결과 발표
- **모더레이션** — 공지 등록·고정, 유저 프로필 카드(닉 클릭)·신고·차단/해제(차단 목록), 삭제/타임아웃, 채팅 얼리기, 등급 게이트
- **저속모드 / 입장 표시 on-off** — 전송 간격 카운트다운(매니저 예외), 입·퇴장 메시지 토글
